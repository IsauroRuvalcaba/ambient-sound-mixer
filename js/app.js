...
class AmbientMixer {
  ...
  constructor() {
    ...
    // the two callback functions are triggered in Timer and run here
    // this is called Dependency Injection with Callbacks
    this.timer = new Timer(
      () => this.onTimerComplete(),
      (minutes, seconds) => this.ui.updateTimerDisplay(minutes, seconds),
    );
    this.currentSoundState = {}; // Object that maintains state of volume
  ...
  setupEventListeners() {
    // Handle all clicks with event deligation
  ...
  async toggleSound(soundId) {
    ...
    if (audio.paused) {
      ...
      // to make sure two presets don't play at same time
  ...
  setSoundVolume(soundId, volume) {
    ...
    // Update the sound volume with the scaled volue
    const audio = this.soundManager.audioElements.get(soundId);
  ...
  loadPreset(presetKey, custom = false) {
    ...
    if (custom) {
      ...
      //defaultPresets comes from soundData.js
    ...
    // Set active preset - to change button color when active
    if (presetKey) {
  ...
  deleteCustomPreset(presetId) {
    //function call inside the "if condition" executes the function
